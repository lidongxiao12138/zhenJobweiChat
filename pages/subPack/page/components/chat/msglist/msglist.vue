	...
	<view scroll-y="true" :class="view + ' wrap ' + (isIPX?'scroll_view_X': '')" @tap="onTap" upper-threshold="-50"
		...
		<view>
			<view class="cardPosition">
				<joblist :titleName="dataJob.name" :mustEducation="dataJob.mustEducation"
					:mustExperience="dataJob.mustExperience" :nature="dataJob.nature" :place="dataJob.place"
					:salaryRange="dataJob.salaryRange" :welfare="listWelfareArr[index]" :hrName="dataJob.hrName"
					:hrImg="dataJob.headImg" :hrPosition="dataJob.hrPosition"></joblist>
			</view>
		...
		<view @longtap="onMsgTap(item)" class="message" v-for="item in chatMsg" :key="item.mid" :id="item.mid">
			...
			<view class="main" :class="item.style">
				...
				<image class="avatar" src="/pages/subPack/page/static/chat/theme2x.png" />
				<view class="msg">
					<image class="err" :class="(item.style == 'self' && item.isFail) ?  'show' : 'hide'"
						src="/pages/subPack/page/static/chat/msgerr.png" />

					<image v-if="item.style == 'self'" src="/pages/subPack/page/static/chat/poprightarrow2x.png"
						class="msg_poprightarrow" />
					<image v-if="item.style == ''" src="/pages/subPack/page/static/chat/popleftarrow2x.png"
					...
					<view v-else-if="item.msg.type == 'txt' || item.msg.type == 'emoji'">
						<view class="template" v-for="(d_item, d_index) in item.msg.data" :key="d_index">
							<text :data-msg="item" @tap="clickMsg" v-if="d_item.type == 'txt'" class="msg-text" style="float:left;"
								selectable="true">{{ d_item.data }}</text>

							<image v-if="d_item.type == 'emoji'" class="avatar"
								:src="'/pages/subPack/page/static/chat/faces/' + d_item.data"
